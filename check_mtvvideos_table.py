#!/usr/bin/env python3
"""
Check all YouTube links in the MTvVideos table only
"""
...
def get_all_mtvvideos():
    """Fetch all records from MTvVideos table"""
    all_records = []
    url = f"https://api.airtable.com/v0/{BASE_ID}/{MTVVIDEOS_TABLE_ID}"
    ...
    while url:
        try:
            ...
            # Handle pagination
            if 'offset' in data:
                url = f"https://api.airtable.com/v0/{BASE_ID}/{MTVVIDEOS_TABLE_ID}?offset={data['offset']}"
            else:
                url = None
...
def main():
    print("🎬 Checking MTvVideos Table for Broken YouTube Links")
    print("=" * 60)
    
    # Get all records
    print("📥 Fetching all records from MTvVideos table...")
    records = get_all_mtvvideos()
    ...
    for i, record in enumerate(records, 1):
        ...
        url = fields.get('url')  # lowercase 'url' field
        
        if not url:
            continue
            
        title = fields.get('title', 'Unknown Title')  # lowercase 'title' field
    ...
    if checked_count > 0:
        ...
        print("No videos with URLs found to check")
    ...
    if broken_videos:
        ...
        filename = f"mtvvideos_table_broken_links_{time.strftime('%Y%m%d_%H%M%S')}.json"
        ...
        print("🎉 All YouTube links in MTvVideos table are working!")
